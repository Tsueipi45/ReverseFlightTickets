...
- CLI：`rft search` 支持命令行参数、JSON 输入、表格/JSON 输出、provider 选择、research provider、SQLite 快照保存；`rft watchlist` 支持 add/list/run。
...
- 价格追踪基础：watchlist 模型、内存和 SQLite watchlist 仓库、一次性 watchlist run、降价阈值告警判断、价格趋势摘要。
- 订购辅助基础：booking handoff、购买前检查清单、人工确认订单记录、订单状态和票号字段。
- 测试：覆盖领域模型、CLI、provider mock、Duffel、Amadeus、orchestrator、排序/风险、SQLite、订购/监控、watchlist 和趋势报告。

## 未完成

- Multi-city 候选生成已支持显式 stopover；自动 stopover/路线发现仍未实现。
- 真实汇率源和缓存未接入；费用、行李费、支付费和退改签风险权重仍是基础模型。
- Watchlist 已有 SQLite 持久化和一次性 run；后台定时调度仍未实现。
